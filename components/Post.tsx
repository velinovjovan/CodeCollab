...
const Post = ({ post }: { post: PostType }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 border border-slate-700 hover:border-slate-600 transition-all shadow-xl hover:shadow-2xl">
      ...
      <div className="mb-4">
        <h3
          className={`text-2xl font-bold text-white mb-4 ${showDetails ? "" : "line-clamp-2"}`}
        >
          {post.title}
        </h3>
        <p
          className={`text-gray-300 leading-relaxed whitespace-pre-wrap ${showDetails ? "" : "line-clamp-4"}`}
        >
          {post.content}
        </p>
      </div>
